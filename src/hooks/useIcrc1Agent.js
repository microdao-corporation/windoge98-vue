...
export function useIcrc1Agent(canisterId) {
  ...
  async function fetchMaxSupply() {
    try {
      data.max_supply = await actor.icrc1_total_supply().then((res) => { return res.toString() });
    } catch (e) {
      error.value = e;
  ...
  async function fetchIcrc1BalanceOf(principal, key) {
    try {
      let res = await actor.icrc1_balance_of({owner: Principal.fromText(principal), subaccount: []}).then((res) => { return res.toString() });
      data[key] = res;
    } catch (e) {
      error.value = e;
      console.error('Error fetching data:', e);
